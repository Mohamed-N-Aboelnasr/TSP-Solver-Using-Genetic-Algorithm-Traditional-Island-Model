...
namespace TSP
{
    static class Program
    {
            ...
            static void Main(string[] args)
        {
            ...
            // var initGeneration =SingleThreadedInitGeneration(configuration.AllCities, hyperParameter.PopulationSize);
           ...
           var htmlContent = chart.GetHtml();
           File.WriteAllText("tsp_solution.html", htmlContent);
           Console.WriteLine("Plot saved as tsp_solution.html. Open it in a browser to view.");
           //chart.Show();
           #endregion
           Console.ReadLine();
        }
        ...
        public static Generation SingleThreadedInitGeneration(List<City> cities,int numberOfChromosomes)
        {
            ...
            Generation G = new Generation(){population = chromosomes};
            return G;
        }
        ...
        public static Generation InitiateGeneration(HyperParameter hp,Configuration c)
        {
            ...
            {
                var rnd = new Random(Seed:x.Item1);
                var chrom = new List<City>(x.Item2.OrderBy((y) => rnd.Next()).ToList());
                pop.Add(new Chromosome() {solution =chrom});
        ...
        public class MakeGeneration
        {
            ...
            public List<(SelectionMethod method, Func<List<Chromosome>, HyperParameter,Random,(Chromosome,Chromosome)> select)> selectList;

            public List<(CrossOverMethod method, Func<(Chromosome, Chromosome),Random,Configuration , (Chromosome, Chromosome)> crossover)> crossoverList;

            public List<(MutationMethod method, Func<Chromosome, Random, Chromosome> mutate)> mutatingList;

            public MakeGeneration()
            {
                evalList = new List<(EvalMethod method, Func<Chromosome, Configuration, HyperParameter, Chromosome> evaluate)>()
                    {
                        (EvalMethod.Distance, EvaluateChromosome)
                    };

                selectList = new List<(SelectionMethod method, Func<List<Chromosome>, HyperParameter,Random,(Chromosome,Chromosome)> select)>()
                    {
                        (SelectionMethod.Tournament, TournamentSelection),
                        (SelectionMethod.Roulette, RouletteSelection)
                    };

                crossoverList = new List<(CrossOverMethod method,
                ...
                mutatingList = new List<(MutationMethod method, Func<Chromosome, Random, Chromosome> mutate)>()
                    {
